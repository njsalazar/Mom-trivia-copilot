# Mom-trivia-copilot

Copilot version

## Famous Moms Trivia Game (Millennial Edition)

A ready-to-play trivia set for a mom-themed costume party, designed for millennials (roughly born in the 1980s) with references from 90s/00s nostalgia plus current pop culture.

## How to play

1. Split guests into teams.
2. Read one clue at a time.
3. Teams guess the famous mom.
4. Award 1 point per correct answer.
5. Optional bonus: 1 extra point if a guest is dressed as that person.

## Trivia questions

1. **She voiced Anna in *Frozen* and has two daughters.**  
   **Answer:** Kristen Bell
2. **This former child star from *Lizzie McGuire* is now a mom and author.**  
   **Answer:** Hilary Duff
3. **She starred in *The Notebook* and is a mom with Ryan Gosling.**  
   **Answer:** Eva Mendes
4. **This *Gossip Girl* alum and lifestyle founder is a famous celebrity mom married to Ryan Reynolds.**  
   **Answer:** Blake Lively
5. **She played Blair Waldorf on *Gossip Girl* and is married to Adam Brody.**  
   **Answer:** Leighton Meester
6. **This pop icon behind “Since U Been Gone” now hosts a daytime talk show and has two kids.**  
   **Answer:** Kelly Clarkson
7. **She was in Destiny’s Child and now has three children with Jay-Z.**  
   **Answer:** Beyoncé
8. **This tennis legend and entrepreneur is a famous girl mom.**  
   **Answer:** Serena Williams
9. **This actress from *Fantastic Four* founded The Honest Company and is mom to three.**  
   **Answer:** Jessica Alba
10. **She went from *The Mickey Mouse Club* to global superstardom and is mom to two sons.**  
    **Answer:** Britney Spears
11. **This *Mean Girls* star (Gretchen Wieners) is now a Hallmark favorite and mom.**  
    **Answer:** Lacey Chabert
12. **This singer of “Firework” is mom to daughter Daisy.**  
    **Answer:** Katy Perry
13. **She starred in *The O.C.* and *Hart of Dixie* and is mom to one daughter.**  
    **Answer:** Rachel Bilson
14. **This *Bring It On* star and producer is mom to two daughters.**  
    **Answer:** Gabrielle Union
15. **This *Princess Diaries* actress and producer has two sons.**  
    **Answer:** Anne Hathaway
16. **She became famous on *Laguna Beach*/*The Hills* and now has three kids.**  
    **Answer:** Kristin Cavallari
17. **This *One Tree Hill* star and podcaster is mom to three children.**  
    **Answer:** Hilarie Burton Morgan
18. **This Spice Girl known as Posh is mom to four and now a fashion designer.**  
    **Answer:** Victoria Beckham
19. **This singer behind “Raise Your Glass” and “Just Give Me a Reason” is mom to two kids.**  
    **Answer:** P!nk
20. **This *This Is Us* actress and singer is also a well-known celebrity mom.**  
    **Answer:** Mandy Moore

## Tiebreaker round (hard mode)

1. **Which famous mom co-created Draper James and has a son named Tennessee?**  
   **Answer:** Reese Witherspoon
2. **Which famous mom from *Glee* has a daughter named Ever?**  
   **Answer:** Lea Michele
3. **Which famous mom from *7th Heaven* became a lifestyle and cookbook creator?**  
   **Answer:** Beverley Mitchell

## Costume ideas that fit the game

- Beyoncé (Renaissance-inspired silver look)
- Blake Lively (*Gossip Girl* Serena vibe)
- Hilary Duff (Y2K pop star look)
- Kristen Bell (*Frozen* nod in icy tones)
- Serena Williams (tennis-chic)
- Jessica Alba (clean beauty founder aesthetic)

Tip: Print each trivia question on a card and keep answer keys separate for the host.
